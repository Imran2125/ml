def recaostar(n):
    ...
    while not solvable:
        if len(marked)==len(and_nodes)+len(or_nodes):
            min_cost_least,min_cost_least_group=least_cost_group(and_nodes,or_nodes,marked)
            solvable=True
...
def least_cost_group(and_nodes,or_nodes,marked):
    ...
    for node_pair in and_nodes:
        if not node_pair[0] in marked and not node_pair[1] in marked:
            cost=0
